...
def getWinnerTotalGoals(competition, year):
    url = ("https://jsonmock.hackerrank.com/api/football_competitions?competition={}&year={}".format(competition, year))
    response = requests.get(url).json()
    # print(response)
    # print(response['data'])
    # print(response['data'][0]['winner'])
    CompetitionWinner = str(response['data'][0]['winner'])

    urlMatchesTeam = (
        "https://jsonmock.hackerrank.com/api/football_matches?competition={}&year={}&team='CompetitionWinner'".format(
            competition, year))
    responseTeam = requests.get(urlMatchesTeam).json()
    total_pages = responseTeam['total_pages']

    totalGoals = 0

    for i in range(1, total_pages + 1):
        url = (
            "https://jsonmock.hackerrank.com/api/football_matches?competition={}&year={}&team='CompetitionWinner'&page={}".format(
                competition, year, i))
        response = requests.get(url).json()
        print(response)

        for j in range(0, len(response['data'])):
            if (response['data'][j]['team1'] == CompetitionWinner):
                totalGoals += int(response['data'][j]['team1goals'])
            elif (response['data'][j]['team2'] == CompetitionWinner):
